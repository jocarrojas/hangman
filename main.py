...
def txts(name):
  global lan
  if lan=="eng":
    print(txt['lan']['eng'][name])
  else:
    print(txt['lan']['spa'][name])

def wSel(diff):
  global listOfWords
  global listOfWordsSpaE
  global listOfWordsSpaM
  global listOfWordsSpaH
  if diff==1:
...
engE=('Also Aged Back Ball Bend Come Card Chat Cook Each Ever Four Feel Fine Gone Have Hear Jump Life Main More Open Pull Able Away Bank Base Bell Came Coat Cash Cool Evil Face Five Fell Fish Gold Hair Into Kick Like Move Nose Only Sell Acid Baby Been Busy Bird Calm City Crow Dark Even Fact Fair Fire Game Girl Here Iron Kill Love Meet Near Push Sale ').split()

engM=('ant baboon badger bat bear beaver camel cat clam cobra cougar coyote crow deer dog donkey duck eagle ferret fox frog goat goose hawk lion lizard llama mole monkey moose mouse mule newt otter owl panda parrot pigeon python rabbit ram rat raven rhino salmon seal shark sheep skunk sloth snake spider stork swan tiger toad trout turkey turtle weasel whale wolf wombat zebra ').split()

engH=('Connecticutian Jackasseries Impedimenta Tergiversation     Polyphiloprogenitive Embourgeoisement Xenotransplantation Myrmecophilous Omphaloskepsis Trichotillomania Psychotomimetic Consanguineous Pulchritudinous Sesquipedalian Onomatopoeia Worcestershire').split()
...
eng={
  ...
  'error':'Error! Call 911 for more info' }
spa={
  'lanSel':'Elegiste español', 
  'difficulty':'Elige la dificultad:\n\n1. Fácil\n2. Medio\n3. Difícil\n4. Escribe una palabra de tu elección\n\n', 
  'unsupported':'No soportado', 
  'error':'¡Error! Llama a emergencias para más información'}
txt={}
txt[lan]={'eng':eng, 'spa':spa}
...
while True:
  if lan==1 or lan==english:
    lan="eng"
    break
  elif lan==2 or lan==spanish:
    lan="spa"
    break
  else:
    print('Not available')
